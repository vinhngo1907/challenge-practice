...
List list_connected_components(Graph*);
...
int main(){
	...
	list = list_connected_components(&G);
	int size = list.size;
	printf ("\n So thanh phan lien thuoc cua do thi la: %d", size);
	for(i=1;i<=size;++i){
		printf ("\n Thanh phan %d - %d dinh", i, element_at(&list, i));
	}
...
List list_connected_components(Graph* G) {
