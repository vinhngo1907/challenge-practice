...
void list_doibong(Graph G, int*List, int type)
{
	int i;
	for(i = 1; i <= G.n; i++)
	{
...
int num[100], min_num[Max_V], on_stack;
Stack s;
int min(int x, int y)
{
	return (x<y)?x:y;
...
/*
	if(is_bigraph(&G))
	{
		printf("Doi bong 1: "); list_doibong(G,color,0);
		printf("\nDoi bong 2: "); list_doibong(G,color,1);
	}
	else
	{
		printf("IMPOSSIBLE");
